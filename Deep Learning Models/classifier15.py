...
for i in range(1,2):
    ...
    data_path = '../../data/dataset0/data' + str(i) + '.mat'
    ...
    data_path = '../../data/dataset1/data' + str(i) + '.mat'
    data = loadmat(data_path)
    ...
    data_path = '../../data/dataset2/data' + str(i) + '.mat'
    data = loadmat(data_path)
    ...
    data_path = '../../data/dataset3/data' + str(i) + '.mat'
    data = loadmat(data_path)
    ...
    for i in range(epochs):
        ...
        for idx_batch, (x, y) in enumerate(train_bar):
            # 将训练数据输入模型
            netout, feature = model(x.to(device))
            netout = netout.squeeze(axis=2)
            # 计算损失函数
        ...
        test_netout, feature = model(test_dataset.dataset[:][0].to(device))
        ...
        if np.max(acc) == accuracy_score(test_trues, test_pres):
    ...
    # 绘制tsne图
    tSNE.plt_tsne(best_feature, test_trues_np, 30, save_name='../../res/fig/EEG_tsne')
    print(best_score)
...
scores_pd.to_csv('/root/autodl-tmp/SSVEP/code/res/res15_b' + str(batch_size) + '_c' + str(len(data_x[0])) + '.csv')
