...
for j in range(1,10):
    ...
    for i in range(4):
        ...
        data_path = 'F:\\D613\\国康\\柴老师\\项目\\SSVEP分类/data/dataset' + str(i) + '/data' + str(j) + '.mat'
    ...
    data_path = 'F:\\D613\\国康\\柴老师\\项目\\SSVEP分类/data/dataset0/data' + str(i) + '.mat'
    data = loadmat(data_path)
    channels = data['X0'][0][0]['chanlocs'][0]['labels'][:]
    sel_channels = [np.where(channels == name)[0][0] for name in sel_channels_name]
    all_data_x0 = data['X0'][0][:]['data']
    ...
    data_path = 'F:\\D613\\国康\\柴老师\\项目\\SSVEP分类/data/dataset1/data' + str(i) + '.mat'
    data = loadmat(data_path)
    ...
    data_path = 'F:\\D613\\国康\\柴老师\\项目\\SSVEP分类/data/dataset2/data' + str(i) + '.mat'
    data = loadmat(data_path)
    ...
    data_path = 'F:\\D613\\国康\\柴老师\\项目\\SSVEP分类/data/dataset3/data' + str(i) + '.mat'
    data = loadmat(data_path)
    ...
    model = EegNet(chunk_size= len(all_data_x[0,0]),
                     num_electrodes = len(all_data_x[0]),
                     F1 = 8,
                     F2 = 16,
                     D = 2,
                     num_classes = 4,
                     ...
                     kernel_2 = 16,
                     dropout = 0.25)
    model15 = EegNet(chunk_size=len(all_data_x15[0, 0]),
                   num_electrodes=len(all_data_x15[0]),
                   F1=8,
                   F2=16,
                   D=2,
                   num_classes=15,
    ...
    for i in range(epochs):
        ...
        for idx_batch, (x, y) in enumerate(train_bar15):
            # 将训练数据输入模型
            netout = model15(x.to(device)).squeeze(axis=2)
            # 计算损失函数
        ...
        test_netout15 = model15(test_dataset15.dataset[:][0].to(device)).squeeze(axis=2)
        test_netout4 = model(test_dataset.dataset[:][0].to(device)).squeeze(axis=2)
...
scores_pd.to_csv('F:\\D613\\国康\\柴老师\\项目\\SSVEP分类\\code/res60_2model_b' + str(batch_size) + '_c' + str(len(data_x[0])) + '.csv')
