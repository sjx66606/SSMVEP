...
for j in range(1,10):
    ...
    model = LMDA(chans=len(all_data_x[0]),
                 ...
                 depth=9)
    model15 = LMDA(chans=len(all_data_x15[0]),
                   num_classes=15,
                   samples=len(all_data_x15[0, 0]),
                   depth=9).cuda()
...
scores_pd.to_csv('/root/autodl-tmp/SSVEP/code/res/lmda_res60_2model_b' + str(batch_size) + '_c' + str(len(data_x[0])) + '.csv')
